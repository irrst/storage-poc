...
/// A PoC Box.
pub struct RawBox<T: ?Sized + Pointee, S: ElementStorage> {
    storage: ManuallyDrop<S>,
    handle: S::Handle<T>,
}

impl<T: Pointee, S: ElementStorage> RawBox<T, S> {
...
impl<T: ?Sized + Pointee, S: ElementStorage> RawBox<T, S> {
    ...
    pub fn try_in<NS: ElementStorage>(
        this: Self,
        mut new_storage: NS,
        ...
        unsafe {
            ptr::copy_nonoverlapping(data.as_ptr() as *const u8, new_data.as_ptr(), layout.size())
        };
...
impl<T: ?Sized + Pointee, S: ElementStorage> Deref for RawBox<T, S> {
    type Target = T;
...
impl<T: ?Sized + Pointee, S: ElementStorage> DerefMut for RawBox<T, S> {
...
impl<T: ?Sized + Pointee, S: ElementStorage> Drop for RawBox<T, S> {
    fn drop(&mut self) {
        //  Safety:
        //  -   There is a value stored, as per constructor's invariants.
...
impl<T: ?Sized + Pointee + Debug, S: ElementStorage> Debug for RawBox<T, S> {
...
mod test_inline {
    ...
    use crate::inline::NonTrackingElement;

    use super::*;

    #[test]
    fn sized_storage() {
        let storage = NonTrackingElement::<u8>::new();
    ...
    #[test]
    fn slice_storage() {
        let storage = NonTrackingElement::<[u8; 4]>::new();
...
mod test_allocator {
    ...
    #[test]
    fn sized_failure() {
        let storage = AllocStorage::new(NonAllocator);
        RawBox::new(1, storage).unwrap_err();
    }
